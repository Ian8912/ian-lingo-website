// Plain server component — no browser-only hooks needed, so no "use client".
import Link from "next/link";

// Same routes as the Navbar so the footer stays in sync with site navigation.
const navLinks = [
  { name: "Home", href: "/" },
...
// Social/contact links shown as small icon buttons in the footer.
const socials = [
  {
    name: "GitHub",
...
export default function Footer() {
  // Computed at render time; on Amplify this updates with each build/deploy.
  const year = new Date().getFullYear();

  return (
    <footer className="mt-20 w-full border-t border-gray-700 bg-black">
      ...
      <div className="border-t border-gray-800 py-4 text-center text-sm text-gray-500">
        © {year} Ian Lingo. All rights reserved.
      </div>
    </footer>
  );
}
